...
class InitSchema1773000000000 {
    ...
    async up(queryRunner) {
        ...
        await queryRunner.query(`CREATE TABLE \`exercise_tag_map\` (\`map_id\` int NOT NULL AUTO_INCREMENT, \`exercise_id\` int NOT NULL, \`symptom_name\` enum ('BACK_PAIN', 'FATIGUE', 'NAUSEA', 'DIZZINESS', 'SWELLING', 'CONSTIPATION', 'PELVIC_PAIN', 'SHORTNESS_OF_BREATH') NOT NULL, \`effect_type\` enum ('POSITIVE_STRONG', 'POSITIVE_WEAK', 'NEGATIVE') NOT NULL, UNIQUE INDEX \`unique_exercise_symptom\` (\`exercise_id\`, \`symptom_name\`), PRIMARY KEY (\`map_id\`)) ENGINE=InnoDB`);
