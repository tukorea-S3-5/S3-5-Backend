"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SymptomController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const passport_1 = require("@nestjs/passport");
const symptom_service_1 = require("./symptom.service");
const create_symptom_dto_1 = require("./dto/create-symptom.dto");
let SymptomController = class SymptomController {
    symptomService;
    constructor(symptomService) {
        this.symptomService = symptomService;
    }
    async create(req, dto) {
        const userId = req.user.user_id;
        return this.symptomService.createSymptoms(userId, dto.symptoms);
    }
    async history(req) {
        const userId = req.user.user_id;
        return this.symptomService.getHistory(userId);
    }
};
exports.SymptomController = SymptomController;
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: '증상 입력' }),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, create_symptom_dto_1.CreateSymptomDto]),
    __metadata("design:returntype", Promise)
], SymptomController.prototype, "create", null);
__decorate([
    (0, common_1.Get)('history'),
    (0, swagger_1.ApiOperation)({ summary: '증상 이력' }),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], SymptomController.prototype, "history", null);
exports.SymptomController = SymptomController = __decorate([
    (0, swagger_1.ApiTags)('Symptom'),
    (0, swagger_1.ApiBearerAuth)('access-token'),
    (0, common_1.UseGuards)((0, passport_1.AuthGuard)('jwt')),
    (0, common_1.Controller)('symptom'),
    __metadata("design:paramtypes", [symptom_service_1.SymptomService])
], SymptomController);
//# sourceMappingURL=symptom.controller.js.map